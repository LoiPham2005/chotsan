 ...
 * vàng — xem SKILL.md. Vì vậy bảng dưới đây tránh cả hai màu đó (bóng rổ ngoài
 * đời màu cam, ở đây là tím hồng). Mã màu nằm ở `--sport-*` trong `globals.css`.
 *
 * Nền là MỘT màu nhạt, không chuyển sắc: gradient nhoè khi thu nhỏ và chết khi
 * in đen trắng (SKILL.md §7).
 *
 * ⚠️ Class viết ĐỦ chữ ở đây, không ghép chuỗi (`text-sport-${key}`): Tailwind
 * chỉ sinh CSS cho tên class nó đọc thấy nguyên văn trong mã nguồn.
 */
export type SportStyle = {
  /** Class màu cho biểu tượng / chữ của môn. */
  text: string;
  /** Class nền nhạt của môn — ô vuông biểu tượng, ảnh sân chưa có. */
  tint: string;
  /** Các nét SVG trong khung 24×24. */
  glyph: React.ReactNode;
};

const SPORT_STYLES: Record<string, SportStyle> = {
  badminton: {
    text: "text-sport-badminton",
    tint: "bg-sport-badminton-tint",
    glyph: (
      <>
  ...
  pickleball: {
    text: "text-sport-pickleball",
    tint: "bg-sport-pickleball-tint",
    glyph: (
      <>
  ...
  "table-tennis": {
    text: "text-sport-table-tennis",
    tint: "bg-sport-table-tennis-tint",
    glyph: (
      <>
...
const DEFAULT_STYLE: SportStyle = {
  text: "text-sport-other",
  tint: "bg-sport-other-tint",
  glyph: <rect x="3" y="5" width="18" height="14" rx="2" />,
};

export function sportStyle(sportKey: string): SportStyle {
  return SPORT_STYLES[sportKey] ?? DEFAULT_STYLE;
}
