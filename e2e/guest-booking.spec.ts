...
import { login, PASSWORD, ACCOUNTS } from "./helpers";
...
test.describe("Khách đặt sân", () => {
  ...
  test("thẻ sân và trang chi tiết có ảnh thật, tải được qua bộ tối ưu ảnh", async ({ page }) => {
    ...
    const cardImage = page.locator('a[href^="/venues/"] img').first();
    ...
    const gallery = page.getByRole("region", { name: /^Ảnh Nhà thi đấu Cầu lông Thành Công/ });
    await expect(gallery.locator("img")).toHaveCount(4);
    const coverImage = gallery.locator("img").first();
    await expect
      .poll(() => coverImage.evaluate((img: HTMLImageElement) => img.naturalWidth))
      .toBeGreaterThan(0);
    await expect(coverImage).toHaveAttribute("alt", /Nhà thi đấu Cầu lông Thành Công — ảnh 1/);
  });
  ...
  test("chọn ô rồi mới đăng nhập — quay về còn nguyên các ô và đặt tiếp được", async ({ page }) => {
    ...
    const freeSlot = page.locator('button[data-minute][aria-pressed="false"]:not([disabled])');
    const selectedSlots = page.locator('button[data-minute][aria-pressed="true"]');
    await expect(freeSlot.first()).toBeVisible({ timeout: 20_000 });
    ...
    await page.locator('input[name="identifier"]').fill(ACCOUNTS.customer);
  ...
  test("bấm ngày khác trên dải ngày thì đổi lịch — không 404, không mang ô của ngày cũ", async ({
    ...
    const otherDay = page.getByRole("group", { name: "Chọn ngày" }).getByRole("link").nth(2);
  ...
  test("mã đặt sân không tồn tại thì 404, không phải trang trống", async ({ page }) => {
    // Màn thanh toán bắt đăng nhập (chỉ người đặt và nhân sự sân được xem), nên
    // phải đăng nhập trước — không thì đây chỉ đo được cú chuyển sang /login.
    await login(page, ACCOUNTS.customer);
    const response = await page.goto("/bookings/KHONGCO");
    expect(response?.status()).toBe(404);
  });

  test("chưa đăng nhập mở màn thanh toán thì về trang đăng nhập, không lộ thông tin khách", async ({
    page,
  }) => {
    await page.goto("/bookings/KHONGCO");
    await expect(page).toHaveURL(/\/login\?next=%2Fbookings%2FKHONGCO/);
  });
});
