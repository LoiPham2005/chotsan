...
import { login, PASSWORD, openFirstVenue, ACCOUNTS } from "./helpers";
...
test.describe("Đích sau khi đăng nhập", () => {
  const LANDINGS: [string, RegExp][] = [
    [ACCOUNTS.admin, /\/venue-approvals/],
    [ACCOUNTS.owner, /\/manage/],
    [ACCOUNTS.staff, /\/manage/],
    [ACCOUNTS.customer, /\/$/],
  ];

  for (const [email, expected] of LANDINGS) {
    test(`${email} → ${String(expected)}`, async ({ page }) => {
  ...
  test("`?next=` LUÔN thắng đích mặc định của vai", async ({ page }) => {
    ...
    await page.locator('input[name="identifier"]').fill(ACCOUNTS.admin);
...
test.describe("Đăng xuất", () => {
  test("xoá phiên và không vào lại được khu cần đăng nhập", async ({ page, context }) => {
    ...
    // Chờ THAY ĐỔI TRẠNG THÁI, không chờ URL: `login` đã đưa về "/" rồi,
    ...
    const hasSession = (await context.cookies()).some(
      (c) => c.name === "session" && c.value !== "",
    );
    expect(hasSession, "cookie phiên phải bị xoá").toBe(false);
...
test.describe("Nhân viên sân", () => {
  test("vào được lịch sân nhưng KHÔNG vào được nhân sự", async ({ browser }) => {
    ...
    const venueId = await openFirstVenue(ownerPage);
    ...
    const scheduleResponse = await page.goto(`/manage/${venueId}`);
    expect(scheduleResponse?.status(), "nhân viên phải trực được sân").toBe(200);

    // `member:manage` không nằm trong bộ quyền mặc định của nhân viên.
    const staffResponse = await page.goto(`/manage/${venueId}/staff`);
