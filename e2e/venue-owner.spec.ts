...
import { login, openFirstVenue, ACCOUNTS } from "./helpers";
...
test.describe("Chủ sân", () => {
  ...
  test("lịch sân hiện lượt đặt kèm nút thao tác", async ({ page }) => {
    const venueId = await openFirstVenue(page);
  ...
  test("hàng chờ duyệt tiền nói rõ phải đối chiếu ngân hàng", async ({ page }) => {
     ...
     */
    const pendingCount = await page.getByRole("button", { name: /Đã nhận đủ tiền/ }).count();
  ...
  test("bảng tick quyền KHÔNG có ba quyền nguy hiểm", async ({ page }) => {
    ...
    const permissionButton = page.getByRole("button", { name: /^Quyền \(/ });
    if ((await permissionButton.count()) === 0) return;
